...
To use the Corporation Search API, an Appkey and SecretKey are required.

The Appkey is a unique authentication key issued for each NHN Cloud service and is used to identify the service and validate API requests. The SecretKey is a secret key that controls access to the API.
